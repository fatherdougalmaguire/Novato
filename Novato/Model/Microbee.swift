...
actor microbee
{
    ...
    private var benchmarkStartClock: ContinuousClock.Instant?
    private var benchmarkStartTStates: UInt64 = 0

    var snapshots: AsyncStream<microbeeSnapshot>
    {
        snapshotsStream
    }
...
//                print("T-states/frame:", tStatesPerFrame)
            ...
            while executedTStates < frameTStates
            {
                ...
                if emulatorState != .running && emulatorState != .halted
                {
                    break
                }
            ...
            {
                if clock.now - nextFrame > frameDuration
                {
                    nextFrame = clock.now
                }
            }
...
//            }
    ...
    private func executeCBInstructions(opcode2: UInt8) -> UInt8
    {
        var tStates : UInt8
    ...
    private func executeDDInstructions(opcode2: UInt8, opcode3: UInt8, opcode4: UInt8) -> UInt8
    {
        var tStates : UInt8
       ...
       case 0xBE: // CP (IX+$d) - DD BE d - Subtracts the value pointed to by IX plus $d from A and affects flags according to the result. A is not modified
        ...
        case 0xBF: // Undocumented - CP A - DD BF - Subtracts A from A and affects flags according to the result. A is not modified
            ...
            tStates = 4
            incrementR(opcodeCount:2)
        case 0xCB: tStates = executeDDCBInstructions(opcode3: opcode3, opcode4: opcode4)
    ...
    private func executeDDCBInstructions(opcode3 : UInt8, opcode4: UInt8) -> UInt8
    {
        var tStates : UInt8
    ...
    private func executeEDInstructions(opcode2: UInt8, opcode3: UInt8, opcode4: UInt8) -> UInt8
    {
        var tStates : UInt8
    ...
    private func executeFDCBInstructions(opcode3: UInt8, opcode4: UInt8) -> UInt8
    {
        var tStates : UInt8
    ...
    private func executeFDInstructions(opcode2: UInt8, opcode3: UInt8, opcode4: UInt8) -> UInt8
    {
        var tStates : UInt8
        ...
        case 0x75: // LD (IY+$d),L - FD 75 d - Stores L to the memory location pointed to by IY plus $d
            logInstructionDetails(instructionDetails: "LD (IY+$d),L", opcode: [0xFD,0x75], values: [opcode3], programCounter: registers.PC)
            let tempResult = registers.IY &+ UInt16(bitPattern: Int16(Int8(bitPattern: opcode3)))
            bus.writeByte(address: tempResult, value: registers.L)
            registers.WZ = tempResult
            registers.PC = registers.PC &+ 3
        ...
        case 0xBF: // Undocumented - CP A - FD BF - Subtracts A from A and affects flags according to the result. A is not modified
            ...
            tStates = 4
            incrementR(opcodeCount:2)
        case 0xCB: tStates = executeFDCBInstructions(opcode3: opcode3, opcode4: opcode4)
    ...
    private func executeInstructions() -> UInt8
    {
        ...
        case 0x76: // HALT - 76 - Suspends CPU operation until an interrupt or reset occurs
            ...
            registers.Q = 0
            tStates = 4
//            if let start = benchmarkStartClock
//            {
//                let elapsed = start.duration(to: clock.now)
//
//                let executedTStates = totalTStates - benchmarkStartTStates
//
...
//            benchmarkStartTStates = totalTStates
        case 0xCA: // JP Z,nn - CA n n - If the zero flag is set, $nn is copied to PC
            ...
            tStates = 10
            incrementR(opcodeCount:1)
        case 0xCB : tStates = executeCBInstructions(opcode2: opcode2)
       ...
       case 0xDC: // CALL C,$nn - DC n n - If the carry flag is set, the current PC value plus three is pushed onto the stack, then is loaded with $nn.
        ...
        case 0xEC: // CALL PE,$nn - EC n n - If the parity/overflow flag is unset, the current PC value plus three is pushed onto the stack, then is loaded with $nn
            ...
            registers.Q = 0
            incrementR(opcodeCount:1)
        case 0xED : tStates = executeEDInstructions(opcode2: opcode2, opcode3: opcode3, opcode4: opcode4)
        ...
        case 0xFC: // CALL M,$nn - FC n n - If the sign flag is set, the current PC value plus three is pushed onto the stack, then is loaded with $nn
            ...
            registers.Q = 0
            incrementR(opcodeCount:1)
        case 0xFD: tStates = executeFDInstructions(opcode2: opcode2, opcode3: opcode3, opcode4: opcode4)
    ...
    func returnSnapshot(stepping: Bool) -> microbeeSnapshot
    {
