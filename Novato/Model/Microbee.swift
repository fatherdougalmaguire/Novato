...
actor Microbee
{
    ...
    private var z80Queue = Z80Queue()
    
    private var registers = Registers()
 
    private var ports = [UInt8](repeating: 0, count: 256)
    ...
    private var tStates : UInt64 = 0
    
    private var emulatorState : emulatorState = .stopped
    private var executionMode : executionMode = .continuous
    
    private var interruptPending = false

    private var crtc = CRTC()
    
    private var mmu = memoryMapper()

    private var mainRAM = memoryBlock(size: 0x8000, label: "mainRAM")
    private var basicROM = memoryBlock(size: 0x4000, deviceType : .ROM, label: "basicROM")
    private var pakROM = memoryBlock(size: 0x2000, deviceType : .ROM, label: "pakROM")
    private var netROM = memoryBlock(size: 0x1000, deviceType : .ROM, label: "netROM")
    private var videoRAM = memoryBlock(size: 0x800, label: "videoRAM", fillValue: 0x20)
    private var pcgRAM = memoryBlock(size: 0x800, label: "pcgRAM")
    private var colourRAM = memoryBlock(size: 0x800,  label: "colourRAM")
    private var fontROM = memoryBlock(size: 0x1000, deviceType : .ROM,  label: "fontROM")
    ...
    init()
    {
        ...
        videoRAM.fillMemoryFromArray(memValues: [87,101,108,99,111,109,101,32,116,111,32,78,111,118,97,116,111], memOffset : 88)  // Welcome to Novato
    ...
    private func TestFlags ( FlagRegister : UInt8, Flag : Z80Flags ) -> Bool
    
    {
        return FlagRegister & Flag.rawValue != 0
    }
    
    private func UpdateFlags ( FlagRegister : UInt8, Flag : Z80Flags, SetFlag : Bool ) -> UInt8
    {
        if (SetFlag)
        {
    ...
    private func testBit (value: UInt8, bitPosition : UInt8  ) -> Bool
    {
        return (value & (1 << bitPosition)) != 0
    }

    private func returnParity(value: UInt8) -> Bool
    {
    ...
    private func logInstructionDetails(instructionDetails: String = "Unknown opcode", opcode: [UInt8], values: [UInt8] = [], programCounter: UInt16)
    {
        #if DEBUG
    ...
    private func executeInstruction()
    {
        ...
        case 0x00: // NOP - 00 - No operation is performed
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x00])
        ...
        case 0x01: // LD BC,nn - 01 n n - Loads $nn into BC
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x01], dataBytes: [opcode2,opcode3])
        ...
        case 0x02: // LD (BC),A - 02 - Stores A into the memory location pointed to by BC
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x02])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x03: // INC BC - 03 - Adds one to BC
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x03])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 6
            incrementR(opcodeCount:1)
        case 0x04: // INC B
...
//            P/V    Overflow    Set if B was 127 (7F) and became -128 (80) indicating a signed overflow.
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x04])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x05: // DEC B
...
//            P/V    Overflow    Set if B was 127 (7F) and became -128 (80) indicating a signed overflow.
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x05])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x06: // LD B,$n - 06 n - Loads $n into B
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x06], dataBytes: [opcode2])
        ...
        case 0x08: // EX AF,AF' - 08 - Adds one to Exchanges the 16-bit contents of AF and AF'
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x08])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x0A: // LD A,(BC) - 0A - Loads the value pointed to by BC into A
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x0A])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x0B: // DEC BC - 0B - Subtracts one from BC
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x0B])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 6
            incrementR(opcodeCount:1)
        case 0x0E: // LD C,$n - 0E n - Loads n into C
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x0E], dataBytes: [opcode2])
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x10: // DJNZ $d - 10 d - The B register is decremented, and if not zero, the signed value $d is added to PC. The jump is measured from the start of the instruction opcode.
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x10], dataBytes: [opcode2])
            if registers.B != 0
            {
        ...
        case 0x11: // LD DE,$nn - 11 n n - Loads $nn into DE
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x11], dataBytes: [opcode2,opcode3])
            registers.PC = registers.PC &+ 3
            tStates = tStates + 10
            incrementR(opcodeCount:1)
        case 0x12: // LD (DE),A - 12 - Stores A into the memory location pointed to by DE
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x12])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x13: // INC DE - 13 - Adds one to DE
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x13])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 6
            incrementR(opcodeCount:1)
        case 0x16: // LD D,$n - 16 n - Loads $n into D
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x16], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x18: // JR d - 18 d - The signed value $d is added to PC. The jump is measured from the start of the instruction opcode
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x18], dataBytes: [opcode2])
            let signedOffset = Int8(bitPattern: opcode2)
            let displacement = Int16(signedOffset)
            registers.PC = registers.PC &+ UInt16(bitPattern: displacement) &+ 2
            tStates = tStates + 12
        case 0x1A: // LD A,(DE) - 1A - Loads the value pointed to by DE into A
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x1A])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x1B: // DEC DE - 1B - Subtracts one from DE
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x1B])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 6
            incrementR(opcodeCount:1)
        case 0x1E: // LD E,$n - 1E n - Loads $n into E
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x1E], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 7
        case 0x20: // JR NZ,$d - 20 d - If the zero flag is unset, the signed value $d is added to PC. The jump is measured from the start of the instruction opcode
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x20], dataBytes: [opcode2])
        ...
        case 0x21: // LD HL,$nn - 21 n n - Loads $nn into HL
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x21], dataBytes: [opcode2,opcode3])
        ...
        case 0x22: // LD ($nn),HL - 22 n n - Stores HL into the memory location pointed to by $nn.
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x22], dataBytes: [opcode2,opcode3])
            registers.PC = registers.PC &+ 3
            tStates = tStates + 16
            incrementR(opcodeCount:1)
        case 0x23: // INC HL - 23 - Adds one to HL
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x23])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 6
            incrementR(opcodeCount:1)
        case 0x26: // LD H,$n - 26 n - Loads $n into H
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x26], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x28: // JR Z,$d - 28 d - If the zero flag is set, the signed value $d is added to PC. The jump is measured from the start of the instruction opcode
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x28], dataBytes: [opcode2])
            if (TestFlags(FlagRegister:registers.F,Flag:Z80Flags.Zero))
            {
                let signedOffset = Int8(bitPattern: opcode2)
                let displacement = Int16(signedOffset)
        ...
        case 0x2A: // LD HL,($nn) - 2A n n - Loads the value pointed to by $nn into HL
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x2A], dataBytes: [opcode2,opcode3])
            registers.PC = registers.PC &+ 3
            tStates = tStates + 16
            incrementR(opcodeCount:1)
        case 0x2B: // DEC HL - 2B - Subtracts one from HL
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x2B])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 6
            incrementR(opcodeCount:1)
        case 0x2E: // LD L,$n - 2E n - Loads n into L.
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x2E], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x2F: // CPL
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x2F])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x30: // JR NC,d
            logInstructionDetails(instructionDetails: "JR NC,$d", opcode: [0x30], values: [opcode2], programCounter: registers.PC)
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x30], dataBytes: [opcode2])
        ...
        case 0x38: // JR C,d
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x38], dataBytes: [opcode2])
            if (TestFlags(FlagRegister:registers.F,Flag:Z80Flags.Carry))
            {
                let signedOffset = Int8(bitPattern: opcode2)
                let displacement = Int16(signedOffset)
        ...
        case 0x3C: // INC A
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x3C])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x3E: // LD A,n
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x3E], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x70: // LD (HL),B
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x70])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x71: // LD (HL),C
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x71])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x72: // LD (HL),D
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x72])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x73: // LD (HL),E
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x73])
            registers.PC = registers.PC &+ 1
            incrementR(opcodeCount:1)
        case 0x74: // LD (HL),H
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x74])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x75: // LD (HL),L
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x75])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x76: // HALT
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x76])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x77: // LD (HL),A
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x77])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0x78: // LD A,B
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x78])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x79: // LD A,C
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x79])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x7A: // LD A,D
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x7A])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x7B: // LD A,E
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x7B])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x7C: // LD A,H
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x7C])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x7D: // LD A,L
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x7D])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0x7E: // LD A,(HL)
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0x7E])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        case 0xB1: // OR C
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xB1])
            registers.PC = registers.PC &+ 1
            tStates = tStates + 4
            incrementR(opcodeCount:1)
        case 0xC2: // JP NZ,nn
        ...
        case 0xC3: // JP nn
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xC3], dataBytes: [opcode2,opcode3])
            registers.PC = UInt16(opcode3) << 8 | UInt16(opcode2)
            tStates = tStates + 10
            incrementR(opcodeCount:1)
        case 0xCA: // JP Z,nn
            logInstructionDetails(instructionDetails: "JP Z,$nn", opcode: [0xCA], values: [opcode2,opcode3], programCounter: registers.PC)
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xCA], dataBytes: [opcode2,opcode3])
            if (TestFlags(FlagRegister:registers.F,Flag:Z80Flags.Zero))
            {
                registers.PC = UInt16(opcode3) << 8 | UInt16(opcode2)
            }
        ...
        case 0xCB: //start CB opcodes
            switch opcode2
            {
                case 0x47: // BIT 0, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x47])
                    tStates = tStates + 8
                ...
                case 0x4F: // BIT 1, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x4F])
                    incrementR(opcodeCount:2)
                case 0x57: // BIT 2, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x57])
                    incrementR(opcodeCount:2)
                case 0x5F: // BIT 3, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x5F])
                    incrementR(opcodeCount:2)
                case 0x67: // BIT 4, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x67])
                    incrementR(opcodeCount:2)
            case 0x6F: // BIT 5, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x6F])
                    incrementR(opcodeCount:2)
                case 0x77: // BIT 6, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x77])
                    incrementR(opcodeCount:2)
                case 0x7F: // BIT 7, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0x7F])
                    incrementR(opcodeCount:2)
                case 0xC7: // SET 0, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0xC7])
                    incrementR(opcodeCount:2)
                case 0xCF: // SET 1, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0xCF])
                    incrementR(opcodeCount:2)
                case 0xD7: // SET 2, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0xD7])
                    incrementR(opcodeCount:2)
                case 0xDF: // SET 3, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0xDF])
                    incrementR(opcodeCount:2)
                case 0xE7: // SET 4, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0xE7])
                    incrementR(opcodeCount:2)
                case 0xEF: // SET 5, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0xEF])
                    incrementR(opcodeCount:2)
                case 0xF7: // SET 6, A
                    ...
                    z80Queue.addToQueue(address: registers.PC, opCodes: [0xCB,0xF7])
                case 0xFF: // SET 7, A
        ...
        case 0xD3: // OUT (n),A
            ...
            case 0x08:
                ...
                {
                    crtc.crtcRegisters.redBackgroundIntensity = 1  // set global background red intensity to 1 = full
                }
                if !testBit(value: registers.A, bitPosition: 1)
                {
                    crtc.crtcRegisters.redBackgroundIntensity = 0 // set global background red intensity to 0 = half
                }
                if testBit(value: registers.A, bitPosition: 2)
                {
                    crtc.crtcRegisters.greenBackgroundIntensity = 1 // set global background blue intensity to 1 = full
                }
                if !testBit(value: registers.A, bitPosition: 2)
                {
                    crtc.crtcRegisters.greenBackgroundIntensity = 0 // set global background blue intensity to 0 = half
                }
                if testBit(value: registers.A, bitPosition: 3)
                {
                    crtc.crtcRegisters.blueBackgroundIntensity = 1 // set global background green intensity to 1 = full
                }
                if !testBit(value: registers.A, bitPosition: 3)
                {
                    crtc.crtcRegisters.blueBackgroundIntensity = 0 // set global background green intensity to 0 = half
                }
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xD3], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 11
            incrementR(opcodeCount:1)
        case 0xDA: // JP C,nn
            logInstructionDetails(instructionDetails: "JP C,$nn", opcode: [0xDA], values: [opcode2,opcode3], programCounter: registers.PC)
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xDA], dataBytes: [opcode2,opcode3])
            if (TestFlags(FlagRegister:registers.F,Flag:Z80Flags.Carry))
            {
                registers.PC = UInt16(opcode3) << 8 | UInt16(opcode2)
            }
        ...
        case 0xDB: // IN A,(n)
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xDB], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 11
            incrementR(opcodeCount:1)
        case 0xE2: // JP PO,nn
            logInstructionDetails(instructionDetails: "JP PO,$nn", opcode: [0xE2], values: [opcode2,opcode3], programCounter: registers.PC)
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xE2], dataBytes:[opcode2,opcode3])
        ...
        case 0xEA: // JP PE,nn
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xEA], dataBytes: [opcode2,opcode3])
            if (TestFlags(FlagRegister:registers.F,Flag:Z80Flags.Parity_Overflow))
            {
                registers.PC = UInt16(opcode3) << 8 | UInt16(opcode2)
            }
        ...
        case 0xED: // ED instructions
            ...
            case 0xB0:  // LDIR
                ...
                z80Queue.addToQueue(address: registers.PC, opCodes: [0xED,0xB0])
                registers.PC = registers.PC &+ 2
                tStates = tStates + 21
                incrementR(opcodeCount:2)
            case 0x4F: //  LD R, A    ED 4F
                        ...
                        z80Queue.addToQueue(address: registers.PC, opCodes: [0xED,0x4F])
                        registers.PC = registers.PC &+ 2
                        tStates = tStates + 9
            case 0x5F:  // LD A, R
                        //             LD A,R
                        ...
                        z80Queue.addToQueue(address: registers.PC, opCodes: [0xED,0x5F])
                        registers.PC = registers.PC &+ 2
                        tStates = tStates + 9
            default:
                logInstructionDetails(opcode: [0xED,opcode2], programCounter: registers.PC)
                z80Queue.addToQueue(address: registers.PC, opCodes: [0xED,opcode2], dataBytes: [opcode2])
                registers.PC = registers.PC &+ 2
        ...
        case 0xF2: // JP P,nn
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xF2], dataBytes: [opcode2,opcode3])
        ...
        case 0xFA: // JP M,nn
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xFA], dataBytes: [opcode2,opcode3])
            if (TestFlags(FlagRegister:registers.F,Flag:Z80Flags.Sign))
            {
                registers.PC = UInt16(opcode3) << 8 | UInt16(opcode2)
            }
        ...
        case 0xFE: // CP n
            ...
            z80Queue.addToQueue(address: registers.PC, opCodes: [0xFE], dataBytes: [opcode2])
            registers.PC = registers.PC &+ 2
            tStates = tStates + 7
            incrementR(opcodeCount:1)
        default:
            logInstructionDetails(opcode: [opcode1], programCounter: registers.PC)
            z80Queue.addToQueue(address: registers.PC, opCodes: [opcode1])
            registers.PC = registers.PC &+ 1
    ...
    func getState() async -> CPUState
    {
        return CPUState( PC: registers.PC,
                         ...
                         vmR1_HorizDisplayed : crtc.ReadRegister(RegNum: 1),
                         vmR6_VertDisplayed : crtc.ReadRegister(RegNum: 6),
                         vmR9_ScanLinesMinus1 : crtc.ReadRegister(RegNum: 9),
                         vmR10_CursorStartAndBlinkMode : crtc.ReadRegister(RegNum: 10),
                         vmR11_CursorEnd : crtc.ReadRegister(RegNum: 11),
                         vmR12_DisplayStartAddrH : crtc.ReadRegister(RegNum: 12),
                         vmR13_DisplayStartAddrL : crtc.ReadRegister(RegNum: 13),
                         vmR14_CursorPositionH : crtc.ReadRegister(RegNum: 14),
                         vmR15_CursorPositionL : crtc.ReadRegister(RegNum: 15),
                         
                         vmRedBackgroundIntensity: crtc.crtcRegisters.redBackgroundIntensity,
                         vmGreenBackgroundIntensity: crtc.crtcRegisters.greenBackgroundIntensity,
                         vmBlueBackgroundIntensity: crtc.crtcRegisters.blueBackgroundIntensity,
    ...
    func returnSnapshot() async -> microbeeSnapshot
    {
        return microbeeSnapshot(
            ...
            crtcSnapshot: crtcSnapshot(
                R0_HorizTotalMinus1: crtc.crtcRegisters.R0_HorizTotalMinus1,
                R1_HorizDisplayed: crtc.crtcRegisters.R1_HorizDisplayed,
                R6_VertDisplayed: crtc.crtcRegisters.R6_VertDisplayed,
                R9_ScanLinesMinus1: crtc.crtcRegisters.R9_ScanLinesMinus1,
                R10_CursorStartAndBlinkMode: crtc.crtcRegisters.R10_CursorStartAndBlinkMode,
                R11_CursorEnd: crtc.crtcRegisters.R11_CursorEnd,
                R12_DisplayStartAddrH: crtc.crtcRegisters.R12_DisplayStartAddrH,
                R13_DisplayStartAddrL: crtc.crtcRegisters.R13_DisplayStartAddrL,
                R14_CursorPositionH: crtc.crtcRegisters.R14_CursorPositionH,
                R15_CursorPositionL: crtc.crtcRegisters.R15_CursorPositionL,
                redBackgroundIntensity: crtc.crtcRegisters.redBackgroundIntensity,
                greenBackgroundIntensity: crtc.crtcRegisters.greenBackgroundIntensity,
                blueBackgroundIntensity: crtc.crtcRegisters.blueBackgroundIntensity
            ),
